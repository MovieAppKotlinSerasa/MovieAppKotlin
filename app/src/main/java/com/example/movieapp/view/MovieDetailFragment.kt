...
class MovieDetailFragment : BottomSheetDialogFragment() {
    ...
    private val movieObserver = Observer<Movie> { result ->
        ...
        binding.movieDetailGenreTextView.text = result.genreList?.get(0)?.name
        binding.movieDetailAverageVoteTextView.text = result.voteAverage.toString()
        binding.movieDetailTitleTextView.text = result.original_title
